...
This document describes how `0labs-vision` handles your data.
...
- `~/.0labs-vision/config.json` — your chosen backend and extraction preferences
- `~/.0labs-vision/models/` — downloaded Whisper models (Local backend only)
- `~/.0labs-vision/sessions/` — cached video frames when session mode is enabled
- `~/.0labs-vision/downloads/` — cached YouTube video downloads
...
Questions about this policy: open an issue at https://github.com/0labs-in/vision-link/issues or reach out to the maintainers at [0labs-in](https://github.com/0labs-in).
