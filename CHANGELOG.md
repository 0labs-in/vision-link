...
- **Universal MCP Support** — Now works with any MCP-compatible AI platform, not just Claude Code
- **Platform-specific setup guides** — Documentation for Claude Desktop, Claude Code, ChatGPT Desktop, Cursor IDE, and VS Code with GitHub Copilot
- **Example configurations** — Sample config files for each supported platform in `examples/` directory
- **Publishing guide** — Complete npm publishing instructions in `PUBLISHING.md`

### Changed

- **Package renamed** — `0labs-vision` → `vision-link` for npm publishing
- **Repository moved** — Now at `https://github.com/0labs-in/vision-link`
- **Tool descriptions** — Made platform-agnostic (removed Claude-specific language)
- **Documentation rewrite** — README and all docs now emphasize multi-platform support
- **Version bump** — 1.3.2 → 1.4.0 to reflect major feature addition
- **Keywords updated** — Added chatgpt, cursor, vscode, copilot, universal to package.json

### Fixed

- **Error messages** — Updated to be platform-agnostic with links to setup documentation

## [1.3.2] - 2026-05-16

### Changed

- **Rebranded from Uttam Video Vision to 0labs Vision** — Updated all package names, URLs, repository references, and configuration paths from `uttam-video-vision` to `0labs-vision`. Configuration directory changed from `~/.uttam-video-vision/` to `~/.0labs-vision/`.
- **Author updated** — Changed from Hiren to 0labs across all documentation and package metadata.
...
- **Session system** (`enable_index` config) — Persistent sessions at `~/.0labs-vision/sessions/{video-hash}/`. Manifest tracks frames by resolution, deduplicates across calls. Auto-cleanup of expired sessions on server startup via `session_max_age_days`.
...
- **Gemini API backend:** `video_watch` no longer fails with `FAILED_PRECONDITION` on every call. The backend now polls the uploaded file's state via `ai.files.get()` until it reaches `ACTIVE` before calling `generateContent`. Thanks to [@JaredTheHammer](https://github.com/JaredTheHammer) for the precise diagnosis.
...
- MCP server published to npm as [`0labs-vision`](https://www.npmjs.com/package/0labs-vision)
- Plugin `.mcp.json` now invokes the server via `npx -y 0labs-vision@latest` — no local `npm install` or `npm run build` required
...
- Persistent configuration at `~/.0labs-vision/config.json`
